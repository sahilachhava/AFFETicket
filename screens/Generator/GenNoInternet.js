...
export default class NoInternet extends Component{
    ...
    componentDidMount(){
      const generator = this.props.navigation.state.params ? this.props.navigation.state.params.name : null;
        if(generator != null){
          this.setState({genName: this.props.navigation.state.params.name});
          this.setState({genUsername: this.props.navigation.state.params.username});
        }
    }
        ...
        NetInfo.isConnected.addEventListener(
            ...
            if(hasInternetConnection){
                setTimeout(()=>{
                    ...
                    this.props.navigation.navigate("GenDashboard", {username: this.state.genUsername, name: this.state.genName});
                },
                1000);
            }else{
                setTimeout(()=>{
                    this.setState({status : false});
                    this.props.navigation.navigate("NoInternet", {username: this.state.genUsername, name: this.state.genName});
                },
                1000);
            }
        });
